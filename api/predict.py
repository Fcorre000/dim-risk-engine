...
def predict_batch(rows: list[dict]) -> list[dict]:
    """Vectorised `predict_shipment` for a batch of raw row dicts.

    Same per-row output shape as `predict_shipment`, but the three slow stages
    (classifier, conformal regressor, anomaly fusion) run once over the whole
    batch instead of once per row. Used by the streaming `/analyze/stream`
    endpoint where the chunker hands us 1,000 rows at a time.

    The per-row `transform_row` call still happens in Python, which is fine —
    the bottleneck on a 1,600-row invoice is the model calls (~50 ms batched
    vs ~3 s per-row).
    """
    if not rows:
        return []

    clf, reg, feature_cols, _, _ = _load_artifacts()

    # Stack per-row feature DataFrames into one (N, 105) frame
    X = pd.concat([transform_row(r) for r in rows], axis=0, ignore_index=True)
    X = X[feature_cols]

    # Classifier — calibrated probabilities; positive class assumed to be label 1
    proba = clf.predict_proba(X)
    cls_idx = list(clf.classes_).index(1) if 1 in clf.classes_ else 1
    p_dim = proba[:, cls_idx].astype(float)

    # Regression — conformal returns (point_pred, interval) in log space.
    # interval has shape (N, 2, 1) where dim 1 is [lower, upper].
    log_point, log_interval = reg.predict_interval(X)
    charge_pred = np.expm1(log_point).astype(float)
    charge_lo = np.expm1(log_interval[:, 0, 0]).astype(float)
    charge_hi = np.expm1(log_interval[:, 1, 0]).astype(float)

    # Phase 4 — anomaly second opinion (lazy import; degrades to null on missing artifacts)
    try:
        from anomaly import score_batch as _score_batch  # type: ignore
        ascore = _score_batch(X)
        anomaly_score = ascore['anomaly_score'].astype(float)
        anomaly_flagged = ascore['anomaly_flagged'].astype(bool)
    except (ImportError, FileNotFoundError):
        anomaly_score = None
        anomaly_flagged = None
    ...
    for i, row in enumerate(rows):
        ...
        ca = row.get('Net Charge Billed Currency')
        if ca is not None and pd.notna(ca):
            ca = float(ca)
            outside = bool(ca < charge_lo[i] or ca > charge_hi[i])
        else:
            ca = None
            outside = None

        a_score = float(anomaly_score[i]) if anomaly_score is not None else None
        a_flag = bool(anomaly_flagged[i]) if anomaly_flagged is not None else None

        signals = [disagrees is True, outside is True, a_flag is True]
        fired = sum(signals)
        review_recommended = fired >= 1
        if fired >= 2:
            review_priority = 'high'
        ...
        out.append({
            'dim_predicted': dim_pred,
            'dim_probability': float(p_dim[i]),
            'dim_disagrees_with_fedex': disagrees,
            'charge_predicted': float(charge_pred[i]),
            'charge_lower_95': float(charge_lo[i]),
            'charge_upper_95': float(charge_hi[i]),
            'charge_actual': ca,
            'charge_outside_interval': outside,
            'anomaly_score': a_score,
            'anomaly_flagged': a_flag,
            'review_recommended': review_recommended,
            'review_priority': review_priority,
        })
    return out
